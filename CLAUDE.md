...
pnpm sim:projections # regenerate the simulated finish table after changing DSA roadmaps,
                     ...
                     # defaults (lib/domain/plan/*.generated.json; stale files fail pnpm test)
...
- Images come only from the `content-images` bucket (`docs/ops/content-images.md`), never from
  git.
- Generated code lives in `.generated/`: never edited, never committed. One exception:
  `lib/domain/plan/*.generated.json` are generated by `pnpm sim:projections` and committed
  (spec §4.8, §5.11); never edit them by hand, and never delete or ignore them —
  `tools/sim/projections.test.ts` fails when they are stale: `projections.generated.json` by its
  inputs hash and rules version, `sim-inputs.generated.json` compared by value with the inputs
  extracted from the current content (ruling M4-R22).
