...
- **Built with:** ui-ux-pro-max (design-system search, typography, color, UX, chart and shadcn
  ...
  contrast script (Appendix A: 110 checks, 0 failing).
...
| `subtle-foreground` | `#78716C` | `#8C8580` | Meta text ≥ 14 px bold or ≥ 18 px only (3:1 tier) |
...
| `success` / `-soft` / `-soft-foreground` | `#15803D` / `#F0FDF4` / `#166534` | `#4ADE80` / `#0F2A1A` / `#86EFAC` | Done, Strong, solved |
| `warning` / `-soft` / `-soft-foreground` | `#B45309` / `#FFFBEB` / `#92400E` | `#FBBF24` / `#2E2106` / `#FCD34D` | Partial, throttled, paused roadmap, content warnings |
...
Every accent passes 4.5:1 as text on `surface`, as text on its own soft tint, and with
...
- **Calendar heatmap:** five levels on a single teal ramp, from `heat-0` (no study) to `heat-4`
  (most minutes). Light: `#F0EFED`, `#99F6E4`, `#0D9488`, `#0F766E`, `#134E4A`. Dark: `#292524`,
  `#134E4A`, `#0D9488`, `#2DD4BF`, `#99F6E4`. Levels 2–4 are ≥ 3:1 from the empty cell.
  - Level 1 is intentionally faint, so any active day also shows a small centred dot in
    `foreground` at 40 % opacity — "studied" never depends on colour alone.
  - Each cell exposes date + minutes on hover and on keyboard focus; a legend with minute ranges
    sits below; an accessible table ("Xem dạng bảng") lists the same data (ui-ux-pro-max chart
    guidance: heatmaps need labels and a table fallback).
...
- **Breakpoints:** Tailwind defaults (sm 640, md 768, lg 1024, xl 1280). Test at 375, 768, 1024 and
  1440 px.
- **App shell:**
  - **< 1024 px:** top bar (title + account) and a **bottom navigation** with 5 items — Hôm nay,
    Ôn tập, Lộ trình, Tiến độ, Cài đặt — icon + label, 56 px tall plus the safe-area inset
    (`env(safe-area-inset-bottom)`); page content gets matching bottom padding so nothing hides
    behind it.
...
- **Touch targets:** ≥ 44 × 44 px for anything tappable (buttons, list rows, pills that act,
  heatmap cells on mobile expand to 44 px hit areas); ≥ 8 px between adjacent targets.
...
| **Code tabs / Solution** (feature) | Python / Java / Go tabs (remembers the user's language), `font-mono text-sm`, `surface-muted` background, horizontal scroll, never wrapped. Solutions are hidden behind "Xem lời giải" with a confirmation that it will count as "cần gợi ý" when shown before grading. |
...
| **CalendarHeatmap** (pattern) | §3.4. 7 rows × weeks, 12 px cells (desktop) / 10 px (mobile, 44 px hit area on tap), legend, table fallback. |
...
- [x] Text contrast ≥ 4.5:1, large/meta text ≥ 3:1, controls and focus ring ≥ 3:1 — **verified for
  every token pair in both themes** (Appendix A).
...
### Light theme — 55 checks, 0 failing
...
| subtle-foreground on surface (large/meta ≥3) | 4.80 | 3.0 | pass |
| border-strong (input/control outline) on surface | 3.69 | 3.0 | pass |
| border-strong on background | 3.54 | 3.0 | pass |
| ring on surface | 3.74 | 3.0 | pass |
| ring on background | 3.59 | 3.0 | pass |
| primary-foreground on primary | 5.47 | 4.5 | pass |
| primary-foreground on primary-hover | 7.58 | 4.5 | pass |
| primary as text/link on surface | 5.47 | 4.5 | pass |
| primary as text/link on background | 5.24 | 4.5 | pass |
| primary-soft-foreground on primary-soft | 7.27 | 4.5 | pass |
| success-foreground on success | 5.02 | 4.5 | pass |
| success as text on surface | 5.02 | 4.5 | pass |
| success-soft-foreground on success-soft | 6.81 | 4.5 | pass |
| warning-foreground on warning | 5.02 | 4.5 | pass |
| warning as text on surface | 5.02 | 4.5 | pass |
| warning-soft-foreground on warning-soft | 6.84 | 4.5 | pass |
| danger-foreground on danger | 6.47 | 4.5 | pass |
| danger as text on surface | 6.47 | 4.5 | pass |
| danger-soft-foreground on danger-soft | 7.60 | 4.5 | pass |
| track-1 text on surface | 7.90 | 4.5 | pass |
| track-1: on-accent text on track-1 | 7.90 | 4.5 | pass |
| track-1 text on track-1-soft | 7.07 | 4.5 | pass |
| track-2 text on surface | 5.18 | 4.5 | pass |
| track-2: on-accent text on track-2 | 5.18 | 4.5 | pass |
| track-2 text on track-2-soft | 4.88 | 4.5 | pass |
| track-3 text on surface | 7.10 | 4.5 | pass |
| track-3: on-accent text on track-3 | 7.10 | 4.5 | pass |
| track-3 text on track-3-soft | 6.48 | 4.5 | pass |
| track-4 text on surface | 5.93 | 4.5 | pass |
| track-4: on-accent text on track-4 | 5.93 | 4.5 | pass |
| track-4 text on track-4-soft | 5.57 | 4.5 | pass |
| track-5 text on surface | 6.04 | 4.5 | pass |
| track-5: on-accent text on track-5 | 6.04 | 4.5 | pass |
| track-5 text on track-5-soft | 5.53 | 4.5 | pass |
| track-6 text on surface | 6.32 | 4.5 | pass |
| track-6: on-accent text on track-6 | 6.32 | 4.5 | pass |
| track-6 text on track-6-soft | 5.89 | 4.5 | pass |
| track-7 text on surface | 7.08 | 4.5 | pass |
| track-7: on-accent text on track-7 | 7.08 | 4.5 | pass |
| track-7 text on track-7-soft | 6.84 | 4.5 | pass |
| track-8 text on surface | 7.27 | 4.5 | pass |
| track-8: on-accent text on track-8 | 7.27 | 4.5 | pass |
| track-8 text on track-8-soft | 6.99 | 4.5 | pass |
| heat-1 vs heat-0 (non-text ≥3 from level 2 up) | 1.10 | 1.0 | pass |
| heat-2 vs heat-0 (non-text ≥3 from level 2 up) | 3.26 | 3.0 | pass |
| heat-3 vs heat-0 (non-text ≥3 from level 2 up) | 4.76 | 3.0 | pass |
| heat-4 vs heat-0 (non-text ≥3 from level 2 up) | 8.25 | 3.0 | pass |

### Dark theme — 55 checks, 0 failing
